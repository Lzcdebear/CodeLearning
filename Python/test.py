...
def writeLyricTofile(song_lyric,song_name):
    ...
    with open('lyri c /'+'on l y'+'.tex t','a +',encoding='utf-8') as f:
        f.write(song_lyric)
    with open('lyri c/'+ song_name +'.tex t','a +',encoding='utf-8') as f:
        f.write(song_lyric)

def createLyricFile():
    isExists = os.path.exists('MusicClassification/New/lyric')
    if not isExists:
        os.makedirs('lyc')
...
def main():
    ...
    f = open('ly r ic/'+' o nly'+'.t ext' ,'r',encoding='utf-8').read()
